...
class UserService:
    ...
    async def upload_profile_image(
        ...
        if user.profile_image and user.profile_image != DEFAULT_PROFILE_IMAGE:
            old_file_path = os.path.normpath(os.path.join('/app/uploads', user.profile_image.lstrip('/')))
            base_dir = os.path.normpath('/app/uploads')
            if os.path.commonpath([old_file_path, base_dir]) != base_dir:
                logger.error(f'경로 조작 시도 차단: {old_file_path}')
    ...
    async def delete_profile_image(
        ...
        if user.profile_image and user.profile_image != DEFAULT_PROFILE_IMAGE:
            file_path = os.path.normpath(os.path.join('/app/uploads', user.profile_image.lstrip('/')))
            base_dir = os.path.normpath('/app/uploads')
            if os.path.commonpath([file_path, base_dir]) != base_dir:
                logger.error(f'경로 조작 시도 차단: {file_path}')
