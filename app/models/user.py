...
class User(Base):
    ...
    gender = Column(SQLEnum(Gender), nullable=False, comment="성별")
    birth_date = Column(Date, nullable=False, comment="생년월일")
    profile_image = Column(String(500), nullable=True, comment="프로필 이미지 URL")
    
    # 역할 및 권한
    role = Column(SQLEnum(UserRole), nullable=False, default=UserRole.STUDENT, comment="사용자 역할")
    
    # 소셜 로그인
    social_provider = Column(SQLEnum(SocialProvider), nullable=False, default=SocialProvider.EMAIL, comment="소셜 로그인 제공자")
